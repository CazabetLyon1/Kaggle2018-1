...
def plot_numbers_images(images, max_column_qt=DEFAULT_MAX_COLUMN_QT):
    if len(images) <= 0:
...
def plot_numbers_test_results(images, expected_values, predicted_values, max_column_qt=DEFAULT_MAX_COLUMN_QT):
    if len(images) <= 0:
        return  # TODO add error handling (no images)
